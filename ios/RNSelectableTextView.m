...
NSString *const CUSTOM_SELECTORR = @"_CUSTOM_SELECTOR_";
...
-(void) _handleGesture
{
    ...
    for(NSString *menuItemName in self.menuItems) {
        NSString *sel = [NSString stringWithFormat:@"%@%@", CUSTOM_SELECTORR, menuItemName];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    ...
    NSRange match = [sel rangeOfString:CUSTOM_SELECTORR];
    if (match.location == 0) {
